angular.module('reciclaFacilApp')
  .controller('CronogramaController', ['CronogramaService', '$scope', function(CronogramaService, $scope) {
    ...
    vm.loadCronogramas = function() {
      console.log('loadCronogramas chamado');
      vm.loading = true;
      
      // Carrega todos os cronogramas (simplificado - sem latitude/longitude)
      CronogramaService.list().then(function(dados) {
        console.log('Cronogramas carregados:', dados);
        console.log('Dados para formatar:', dados);
        vm.coletas = vm.formatarCronogramas(dados || []);
        console.log('Cronogramas formatados:', vm.coletas);
        vm.loading = false;
      }).catch(function(err) {
    ...
    vm.formatarCronogramas = function(cronogramas) {
      console.log('formatarCronogramas chamado com:', cronogramas);
      if (!cronogramas || cronogramas.length === 0) {
        console.log('Nenhum cronograma para formatar');
        return [];
      }
      
      return cronogramas.map(function(cronograma) {
        console.log('Formatando cronograma:', cronograma);
        var data = new Date();
        ...
        console.log('Cronograma formatado:', formatado);
        return formatado;
      });
    };
